...
struct XcodebuildTool {
    func run(source: String, arguments: [String], verbose: Bool = false) throws {
        ...
        guard let targetBuildSettings = buildSettings.values.first(where: { $0.sources().first { $0.absoluteURL == targetSource.absoluteURL } != nil }) else {
            throw XcodebuildError.sourceFileNotFound(targetSource)
        }
        ...
        if let productType = targetBuildSettings.settings["PRODUCT_TYPE"], productType.hasPrefix("com.apple.product-type.framework") || productType.hasPrefix("com.apple.product-type.library") {
        ...
        if fileSystem.exists(AbsolutePath(targetSource.path)) && fileSystem.isFile(AbsolutePath(targetSource.path)) {
            let dependencies = sources.filter { $0.absoluteURL != targetSource.absoluteURL }
            let processor = SwiftAST(buildOptions: swiftOptions, dependencies: dependencies, bridgingHeader: bridgingHeaderPath)
            let root = try processor.processFile(input: targetSource, verbose: verbose)
            ASTPrinter(sourceFile: targetSource).print(root)
        }
    }
...
struct XcodebuildOptions {
    ...
    init(_ arguments: [String]) throws {
        ...
        while let (index, option) = iterator.next() {
            ...
            case "-enableCodeCoverage", "-testLanguage", "-testRegion":
                ...
                if let argument = iterator.next() {
                    ...
                    throw XcodebuildError.invalidArgument(option)
                }
            ...
            case "-xctestrun":
                throw XcodebuildError.optionNotSupported(option)
            default:
                break
            }
        }
        ...
        if buildActions.isEmpty {
            self.buildActions = ["build"]
        } else {
            self.buildActions = buildActions.map { $0.1 }
        }
        if self.buildActions.filter({ $0 == "build" || $0 == "test" || $0 == "build-for-testing" }).isEmpty {
            throw XcodebuildError.buildActionNotSupported
        }
                                            ...
                                            for fileID in fileIDs {
                                                if let fileObject = objects[fileID] as? [String: Any], let isa = fileObject["isa"] as? String, isa == "PBXBuildFile", let fileRefID = fileObject["fileRef"] as? String, let fileRefObject = objects[fileRefID] as? [String: Any] {
                                                    if let isa = fileRefObject["isa"] as? String, isa == "PBXFileReference", let path = fileRefObject["path"] as? String {
                                                        ...
                                                        for groupPath in groupPaths where !groupPath.isEmpty {
...
private enum XcodebuildError: Error {
...
extension XcodebuildError: CustomStringConvertible {
    var description: String {
        ...
        case .sourceFileNotFound(let url):
            return "source file (\(url.path) not found)"
        }
    }
}
